...
function learned(slug: string, over: Partial<LearnedMaterial> = {}): LearnedMaterial {
  return {
    slug,
    title: slug,
    type: 'paper',
...
function backlog(slug: string, over: Partial<BacklogMaterial> = {}): BacklogMaterial {
  return {
    slug,
    title: slug,
    type: 'video',
...
describe('buildGraph', () => {
  it('分类骨架完整（即使无条目），member/tag 边正确', () => {
    ...
    const catIds = g.nodes.filter((n) => n.kind === 'category').map((n) => n.id);
    ...
    // 跨星系共享标签 x → 一条 tag 边
    expect(g.links).toContainEqual({ source: 'mat:a', target: 'mat:b', kind: 'tag', weight: 1 });
  });
  ...
  it('learning 状态来自 backlog.status，learned 一律 done', () => {
    const g = buildGraph([learned('done1')], [backlog('ing', { status: 'learning' })]);
    const st = (id: string) => g.nodes.find((n) => n.id === id)?.kind === 'material'
...
describe('applyLayout（确定性 + 增量稳定）', () => {
  ...
  it('顶层分类角槽与内容量无关：增删条目不移动任何已有节点', () => {
    const before = applyLayout(
      buildGraph(
        [learned('a', { category: ['Systems', 'Distributed'] }), learned('b', { category: ['ML'] })],
  ...
  it('同扇区相邻子分类的间距 > 双盘直径（防材料盘重叠）', () => {
    ...
    for (let i = 0; i + 1 < names.length; i++) {
      ...
      expect(dist, `${names[i]} 与 ${names[i + 1]} 弦距`).toBeGreaterThan(80 * 2);
    }
  });
