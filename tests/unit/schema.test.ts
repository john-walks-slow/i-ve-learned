...
import { backlogSchema, learnedSchema } from "../../src/lib/schema";
...
describe("learnedSchema", () => {
  ...
  it("拒绝非法 url", () => {
    expect(learnedSchema.safeParse({ ...base, url: "not-a-url" }).success).toBe(
      false,
    );
  });
  ...
  it("duration 上限 100h（防手滑）", () => {
    expect(
      learnedSchema.safeParse({ ...base, duration: 60 * 101 }).success,
    ).toBe(false);
  });
});
...
describe("backlogSchema", () => {
  ...
  it("status 只能是 todo|learning", () => {
    expect(backlogSchema.safeParse({ ...base, status: "done" }).success).toBe(
      false,
    );
  });
